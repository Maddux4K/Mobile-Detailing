...
// Exports a React component; no manual createRoot.
...
// Square embed snippet you provided
...
function Booking() {
  useEffect(() => {
    // Load Tailwind for preview environments that don't include it by default
    const id = "tw-cdn";
    ...
    // Inject Square widget immediately
...
// --- Dev smoke tests (non‑fatal) ---
function runDevTests() {
  try {
    // pricing sanity
    console.assert(PRICING.exteriorOnly === 55, "Exterior Only should be $55");
    console.assert(PRICING.standardInterior === 99, "Standard Interior should be $99");
    console.assert(currency(PRICING.exteriorOnly) === "$55", "currency() formats whole dollars");
    // square url sanity
    console.assert(
      SQUARE_WIDGET_SRC.startsWith("https://app.squareup.com/appointments/buyer/widget/40rsi5zy5ksrjt/"),
      "Square widget URL should match provided snippet"
    );
  } catch (e) {
    console.warn("Dev test warning:", e);
  }
}
